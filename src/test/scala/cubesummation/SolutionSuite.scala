...
class SolutionSuite extends FunSuite {
  ...
  def traverseInputs(n: Int): Unit = {
    Option(getClass.getResourceAsStream("/cubesummation/input_" + n)) match {
      case Some(input) => {
        test(s"Result should match with the output file number $n") {
          val answer = scala.io.Source.fromURL(getClass.getResource("/cubesummation/output_" + n)).mkString
