...
class SolutionSuite extends FunSuite {
  ...
  test("Result should match with the output file") {
    ...
    def traverseInputs(n: Int): Unit = Option(getClass.getResourceAsStream("/countingontree/input_" + n)) match {
      case Some(input) => {
        val answer = scala.io.Source.fromURL(getClass.getResource("/countingontree/output_" + n)).mkString
